...
NEXT_PUBLIC_SITE_URL=https://jijiangeshui.com
...
脚本会强制把上传环境中的 `NEXT_PUBLIC_SERVER_URL` 和 `NEXT_PUBLIC_SITE_URL` 设置为 `https://jijiangeshui.com`。GitHub Actions 构建 standalone 产物时也会使用 `SITE_URL` 注入这两个变量，避免 sitemap、robots 和 JSON-LD 在构建阶段回退到 `localhost:4000`。不要把生产密钥提交到 Git，也不要直接使用 `.env.example` 作为生产环境文件。
...
`.github/workflows/deploy.yml` 会在 `main` 分支 push 或手动触发时执行 `app` 目录下的依赖安装、Payload 类型生成、lint、测试、类型检查和构建，并把 standalone 构建产物打包上传。构建步骤使用 Repository variable `SITE_URL`（未设置时默认为 `https://jijiangeshui.com`）注入 `NEXT_PUBLIC_SERVER_URL` 和 `NEXT_PUBLIC_SITE_URL`；前台 CMS 页面使用运行时服务端渲染，Workflow 构建阶段不需要连接生产数据库；请求到达生产 Node 服务后，服务端再读取生产环境中的 Payload 数据。检查通过后调用 `npm run deploy:ssr -- --artifact <release.zip>` 完成远程部署；服务器只安装运行依赖、执行数据库 migration、切换 release 和重启服务，不再重复执行 `npm run build`。
...
- `ENV_PRODUCTION`：完整的生产环境变量文件内容，至少包含 `DATABASE_URI`、`PAYLOAD_SECRET`、`PAYLOAD_DB_PUSH=false`、`NEXT_PUBLIC_SERVER_URL=https://jijiangeshui.com` 和 `NEXT_PUBLIC_SITE_URL=https://jijiangeshui.com`；接入 Google Analytics 时再加入 `NEXT_PUBLIC_GA_MEASUREMENT_ID`。
