...
import { getAvailableCityRule, getAvailableCityRuleDataset, getAvailableCityRules, getCityRuleDatasetSummary } from '@/lib/city-rule-service'
import { auditCityRule, getRuleFreshnessLabel, getRuleFreshnessStatus, getRuleQualityStatus } from '@/lib/city-rule-quality'
...
export default async function CityPage({ params }: CityPageProps) {
  ...
  const ruleSourceStatus = getCityRuleSourceStatus({
    city,
    ruleDatasetSummary,
  ...
  const ruleFreshnessStatus = getRuleFreshnessStatus(ruleSourceDate)
  const ruleFreshnessLabel = getRuleFreshnessLabel(ruleFreshnessStatus)
  const hiddenRuleIssueCount = Math.max(ruleQualityIssues.length - 3, 0)
  ...
  return <div className="app-shell"><SiteHeader active="calculator" /><main className={styles.page}>
    ...
    <section className={styles.hero}><div><div className={styles.titleLine}><MapPin size={20} /><span>{rule.label} · {currentYear}年规则</span></div><h1>{rule.label}个税计算器</h1><p>按 {rule.label} {currentYear} 年社保、公积金规则，测算工资到手、个人缴费和全年预扣变化。</p><PrimaryActionLink href={`/calculator?city=${city}`}>开始计算 <ArrowRight size={16} /></PrimaryActionLink></div><div className={[styles.status, styles.ruleStatusCard, styles[`${ruleQualityStatus}RuleStatus`]].join(' ')}>{ruleQualityStatus === 'error' ? <AlertTriangle size={20} /> : <ShieldCheck size={20} />}<div><span>规则状态</span><strong>{ruleQualityLabel}</strong></div><dl><div><dt>规则来源</dt><dd>{ruleSourceStatus.label}</dd></div><div><dt>生效日期</dt><dd>{rule.effective}</dd></div><div><dt>核对日期</dt><dd>{ruleSourceDate}</dd></div><div><dt>核对状态</dt><dd data-freshness={ruleFreshnessStatus}>{ruleFreshnessLabel}</dd></div><div><dt>来源链接</dt><dd>{ruleSourceLinks.length || 0} 个</dd></div></dl>{ruleQualityIssues.length > 0 ? <ul className={styles.ruleIssueList}>{ruleQualityIssues.slice(0, 3).map((issue) => <li key={`${issue.category}-${issue.message}`}>{issue.message}</li>)}{hiddenRuleIssueCount > 0 ? <li>另有 {hiddenRuleIssueCount} 项待处理</li> : null}</ul> : null}<p className={styles.ruleSourceHint}>{ruleSourceStatus.detail}</p></div></section>
