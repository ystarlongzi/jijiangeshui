...
import { currentYear, ruleCheckedDate, siteName } from '@/lib/site'
...
export async function generateMetadata({ params }: CityPageProps): Promise<Metadata> {
  ...
  return rule ? { title: `${rule.label}个税计算器｜${currentYear}年工资税后与五险一金｜${siteName}`, description: `按${rule.label}${currentYear}年社保公积金规则，测算工资到手、五险一金和全年个税变化。`, alternates: { canonical: `/city/${city}` } } : { title: `城市个税计算器｜${siteName}` }
}
...
export default async function CityPage({ params }: CityPageProps) {
  ...
  return <div className="app-shell"><SiteHeader active="calculator" /><main className="city-page">
    ...
    <section className="city-next-step"><div><Calculator size={20} /><div><h2>想知道自己的到手工资？</h2><p>输入税前月薪，查看本月个税、五险一金和全年逐月明细。</p></div></div><Link href={`/calculator?city=${city}`}>进入工资计算器 <ArrowRight size={16} /></Link></section>
    <p className="city-disclaimer">规则核对日期：{ruleCheckedDate}。页面结果仅供测算，最终以个税 APP、扣缴单位或税务机关口径为准。</p>
    <SiteFooter />
  </main></div>
}
