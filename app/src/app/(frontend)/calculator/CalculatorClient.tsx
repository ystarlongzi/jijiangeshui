    ...
    <main id="top" className={styles.pageContent}>
      ...
      <section className="workspace-grid" aria-label="个税计算器">
        <form className="input-panel panel" onSubmit={(event) => { event.preventDefault(); calculate() }}>
          ...
          <div className="deduction-block" id="deduction"><div className="label-with-action deduction-label-row"><label htmlFor="deductionAmount">专项附加扣除</label><Button variant="text" type="button" onClick={() => setDeductionDialogOpen(true)}>选择项目</Button></div><MoneyInput id="deductionAmount" className={isDeductionInvalid ? 'input-error' : ''} value={deductionAmount} onChange={(value) => { setDeductionAmount(value); setDeductionSelections({}) }} /><p className="field-meta">{selectedDeductionItems.length > 0 ? `已选择 ${selectedDeductionItems.map((item) => item?.label).join('、')}。` : '可直接输入本月扣除总额，也可以按项目选择后自动回填。'}</p>{isDeductionInvalid && <p className="field-error">这里填写的是本月扣除额，不能高于税前月薪。</p>}</div>
...
function BaseField({ label, value, min, max, invalid, editing, onEdit, onChange }: { label: string; value: number; min: number; max: number; invalid: boolean; editing: boolean; onEdit: () => void; onChange: (value: number) => void }) {
  ...
  return <div className="field-block"><div className="label-with-action"><label>{label}</label><Button className={styles.editBaseButton} variant="text" type="button" aria-pressed={editing} onClick={onEdit}>{editing ? '完成' : '编辑'}</Button></div><MoneyInput className={`small-input${invalid ? ' input-error' : ''}`} value={value} min={min} max={max} readOnly={!editing} onChange={onChange} /><p className="field-meta">允许范围：{range(min, max)}</p>{invalid && <p className="field-error">{label}需要在允许范围内。</p>}</div>
}
