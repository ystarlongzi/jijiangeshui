...
import styles from './BonusTaxClient.module.css'
...
export default function BonusTaxClient() {
  ...
  return <>
  <div className="app-shell"><SiteHeader active="bonus-tax" /><main className={styles.page}>
    <header className={styles.hero}><div><div className={styles.eyebrow}><Gift size={18} />{currentYear} 年年终奖个税计算器</div><h1>年终奖，<br />哪种计税更划算？</h1><p>输入年终奖和全年收入，比较单独计税与并入综合所得后的奖金到手金额。</p></div><div className={styles.policyNote}><strong>政策有效期</strong><span>全年一次性奖金单独计税政策执行至 {bonusPolicyEndDate}</span></div></header>
    <section className={styles.workspace} aria-label="年终奖个税计算器">
      <Panel as="form" className={styles.input} onSubmit={(event) => { event.preventDefault(); trackEvent('calculate_complete', { calculator: 'bonus_tax', better: result.better, hasThresholdTip: Boolean(result.separateOptimization) }) }}><h2>计算你的年终奖</h2><BonusInput id="bonus" label="年终奖税前金额" value={bonus} onChange={setBonus} />
...
function ThresholdAlert({ optimization, bonus }: { optimization: NonNullable<ReturnType<typeof calculateBonusTax>['separateOptimization']>; bonus: number }) {
  ...
  return <div className={styles.thresholdAlert}><div className={styles.alertCopy}><Award size={18} /><p>单独计税下，当前奖金跨过税率临界点。把金额调整到 <strong>{money(optimization.suggestedBonus)}</strong>，预计到手反而更多。</p></div><div className={styles.thresholdGrid}><div><span>当前奖金</span><strong>{money(bonus)}</strong><small>到手 {money(optimization.currentTakeHome)}</small></div><div><span>建议金额</span><strong>{money(optimization.suggestedBonus)}</strong><small>到手 {money(optimization.suggestedTakeHome)}</small></div><div><span>到手差额</span><strong>{money(optimization.difference)}</strong><small>少发奖金，少跨档</small></div></div><div className={styles.thresholdChart} aria-label="年终奖临界点到手对比"><div className={styles.thresholdChartRow}><span>当前税前</span><i style={{ width: currentBonusWidth }} /><strong>{money(bonus)}</strong></div><div className={styles.thresholdChartRow}><span>当前到手</span><i className={styles.takehomeBar} style={{ width: currentTakeHomeWidth }} /><strong>{money(optimization.currentTakeHome)}</strong></div><div className={`${styles.thresholdChartRow} ${styles.recommended}`}><span>建议税前</span><i style={{ width: suggestedBonusWidth }} /><strong>{money(optimization.suggestedBonus)}</strong></div><div className={`${styles.thresholdChartRow} ${styles.recommended}`}><span>建议到手</span><i className={styles.takehomeBar} style={{ width: suggestedTakeHomeWidth }} /><strong>{money(optimization.suggestedTakeHome)}</strong></div></div></div>
}
...
function BonusInput({ id, label, value, onChange }: { id: string; label: string; value: number; onChange: (value: number) => void }) {
  return <label className={styles.field} htmlFor={id}><span>{label}</span><MoneyInput id={id} value={value} onChange={onChange} /></label>
}
...
function BonusResultCard({ title, result, active, note }: { title: string; result: ReturnType<typeof calculateBonusTax>['separate']; active: boolean; note: string }) {
  const { money } = useMoneyFormat()
  return <article className={`${styles.resultCard}${active ? ` ${styles.active}` : ''}`}><div className={styles.cardTitle}><h3>{title}</h3>{active && <span className={styles.better}><Award size={16} />更优</span>}</div><div className={styles.cardTakehome}><small>奖金到手</small><strong>{money(result.takeHome)}</strong></div><dl><div><dt>应缴个税</dt><dd>{money(result.tax)}</dd></div><div><dt>适用税率</dt><dd>{Math.round(result.bracket.rate * 100)}%</dd></div></dl><p>{note}</p></article>
}
