...
export default function BonusTaxClient() {
  ...
  <div className="app-shell"><SiteHeader active="bonus-tax" /><main className={styles.page}>
    ...
    <section className={styles.workspace} aria-label="年终奖个税计算器">
      ...
      <section className={`${styles.result} panel`} aria-live="polite"><div className={styles.resultHeading}><div><span className={styles.sectionTitle}>计算结果</span><p>{currentYear} 年 · 年终奖 {money(bonus)}</p></div><span className={styles.badge}>推荐方案</span></div><div className={styles.recommendation}><span>预计更划算</span><strong>{result.better === 'separate' ? '单独计税' : '并入综合所得'}</strong><p>预计多到手 {money(result.difference)}。你可以根据单位发放和年度申报情况进一步确认。</p></div>{result.separateOptimization && <ThresholdAlert optimization={result.separateOptimization} bonus={bonus} />}<div className={styles.comparison}><BonusResultCard title="单独计税" result={result.separate} active={result.better === 'separate'} note="年终奖除以 12 个月确定税率，再单独计算奖金税额。" /><BonusResultCard title="并入综合所得" result={result.combined} active={result.better === 'combined'} note="年终奖并入全年综合所得，按年度税率计算增量税额。" /></div><p className={styles.resultNote}>以上为居民个人全年一次性奖金测算，不适用于劳务报酬、股权激励等其他收入类型。</p><div className="result-actions"><a className="link-button" href="/tax-rate">查看税率表 <span>→</span></a><button className="link-button icon-link-button" type="button" onClick={exportCsv}><Download size={14} />导出 CSV</button><button className="link-button icon-link-button" type="button" onClick={copyResult}><Copy size={14} />复制结果</button><button className="link-button icon-link-button" type="button" onClick={copyShareLink}><Copy size={14} />复制链接</button></div></section>
    </section>
