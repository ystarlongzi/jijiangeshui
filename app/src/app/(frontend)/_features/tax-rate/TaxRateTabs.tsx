...
export default function TaxRateTabs() {
  ...
  return <section className={styles.tabsSection} aria-label="个人所得税税率表">
    ...
    <Panel className={styles.tabPanel} role="tabpanel"><div className={styles.tableHeading}><div><h2>{active.title}</h2><p>{active.description}</p></div><span className={styles.yearLabel}>{currentYear} 年</span></div>{active.rows ? <DataTable ariaLabel={active.title} columns={rateColumns} rows={rateRows} wrapperClassName={styles.tableWrap} tableClassName={styles.table} /> : <div className={styles.simpleRatePanel}><div className={styles.simpleRateCopy}><strong>{active.rate}</strong><p>{active.note}</p></div></div>}{active.rows && <div className={styles.tabNote}><Info size={15} /><span>{active.note}</span></div>}<div className={styles.sourceLine}><span>来源：国家税务总局法规库；规则核对日期：{ruleCheckedDate}</span><a href="https://fgk.chinatax.gov.cn/zcfgk/c100012/c5194838/content.html" target="_blank" rel="noreferrer">查看个人所得税法 <ExternalLink size={13} /></a></div></Panel>
